...
contract Token is ViaToken, ERC20, Initializable, OwnableUpgradeable {
    ...
    function addTotalSupply(bytes16 amount) override external{
        require(issuer==msg.sender);
        //adjust total supply
        totalSupply_ = ABDKMathQuad.add(totalSupply_, amount);
    }

    function reduceSupply(bytes16 amount) override external{        
        require(issuer==msg.sender);
        totalSupply_ = ABDKMathQuad.sub(totalSupply_, amount);
    }

    function addBalance(address party, bytes16 amount) override external{
        require(issuer==msg.sender);
        //add via to this contract's balance first (ie issue them first)
        balances[party] = ABDKMathQuad.add(balances[party], amount);
    }

    function reduceBalance(address party, bytes16 amount) override external{
        require(issuer==msg.sender);
    ...
    function transferToken(address sender, address receiver, uint256 tokens) override external returns (bool){
        require(issuer==msg.sender);
    ...
    function requestTransfer(address receiver, uint tokens) override external returns (bool){
        require(issuer==msg.sender);

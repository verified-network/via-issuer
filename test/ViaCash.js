...
contract("IssuingViaUSD", async (accounts) => {
    it("should send ether to Via-USD cash contract and then get some Via-USD cash tokens", async () => {
        ...
        let callbackToViaOracle = getFirstEvent(oracle.LogResult({fromBlock:'latest'}));
        callbackToViaOracle.then(function(){
          truffleAssert.createTransactionResult(oracle, callbackToViaOracle.transactionHash);
        }).then(function(){
          console.log("Via oracle ether balance after query:", web3.eth.getBalance(oracle.address));
          console.log("Account Via-USD cash token balance after sending ether:", web3.utils.hexToNumberString(web3.utils.toHex(viausdCash.balanceOf(accounts[0]))));
        })
...
contract("ViaUSDRedemptionAfterEURExchangeTransferAndRedemption", async (accounts) => {
  it("should send Via-USD to Via-EUR cash contract, get some Via-EUR cash tokens which it should transfer to another account which will redeem Via-EUR to get Via-USD which it will again redeem to get ether", async () => {
    ...
    await viaeurCash.transferFrom(accounts[1], viaeurCashAddress, 50);
